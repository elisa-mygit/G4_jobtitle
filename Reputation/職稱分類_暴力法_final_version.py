...
#人工暴力分類
#在function中丟入中文/英文職稱名以後，return職稱的分類種類(1~4數字)
def category_function(original_job):
  
  #人工分類分為
  #等第四(最終最終return4)：高收入族群，公司經營管理階層
  #等第三(最終最終return3)：一般工作，平均值
  #等第二(最終最終return2)：非正職工作或工作不穩定，收入平均為最低時薪收入平均為最低時薪
  #等第一(最終最終return1)：完全沒有工作、收入零
  ...
  contain_category = []
  if job_title_i == "" or any([n in job_title_i for n in lowest_reputation_job_title]) == True or job_title_i == "家" or all([n in job_title_i for n in meaningless_item]) == True:
      return 1
    ...
    #先看看職稱裡有哪些類別的關鍵字，不直接歸類是因為會有是因為會有ex：助理教授、總經理特助問題
      ...
      if all([n in contain_category for n in ["2"]]) == True:
        ...
        return 3
      elif all([n in contain_category for n in ["4"]]) == True:
        return 4
